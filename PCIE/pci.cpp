        ...
        for (dev = 0; dev <= PCI_MAX_DEV; dev++)
            for (fun = 0; fun <= PCI_MAX_FUN; fun++)
            {
                ...
                if (((data & 0xFFFF) != 0xFFFF) && (data != 0))
                {
                    ...
                    addr = PCI_BASE_ADDR | (bus << 16) | (dev << 11) | (fun << 8) |(3<<2);
                    outl(addr, CONFIG_ADDR);
                    data = inl(CONFIG_DATA);
                    printf("\t\t %x", (data&0xF0000)>>16);
                   
                    addr = PCI_BASE_ADDR | (bus << 16) | (dev << 11) | (fun << 8) |(4<<2);
                    outl(addr, CONFIG_ADDR);
                    data = inl(CONFIG_DATA);
                    printf("\t %10x", data);
                    addr = PCI_BASE_ADDR | (bus << 16) | (dev << 11) | (fun << 8) |(5<<2);
                    outl(addr, CONFIG_ADDR);
                    data = inl(CONFIG_DATA);
                    printf("\t %10x", data);
                    addr = PCI_BASE_ADDR | (bus << 16) | (dev << 11) | (fun << 8) |(8<<2);
                    outl(addr, CONFIG_ADDR);
                    data = inl(CONFIG_DATA);
                    printf("\t %10x", data);
