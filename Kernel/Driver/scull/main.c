 ...
 */
//EPORT_SYMBOL will add 2 sections(__ksymtab __ksymtab_strings) in *.o
//EXPORT_SYMBOL(scull_proc_open);
/*
...
{
    print_jiazi("create dir /proc/scullmem");
...
int scull_open(struct inode *inode, struct file *filp)
{
    ...
    print_jiazi("The Process is --%s (pid-->%i) open the scull device!\n",task->comm,task->pid);
    print_jiazi("export symbol:seq_read @%p\n",seq_read);
    dump_stack();
...
int scull_init_module(void)
{
	...
	dev_t dev = 0;
    //struct task_struct *task=current;
    //print_jiazi("The Process is --%s (pid-->%i)\n",task->comm,task->pid);
    
    //print_jiazi("The swapper_pg_dir is at 0x%x\n",swapper_pg_dir);
/*
 * Get a range of minor numbers to work with, asking for a dynamic
 * major unless directed otherwise at load time.
 */
	if (scull_major) {
        print_jiazi("Major device is set statically %d\n",scull_major);
        ...
        print_jiazi("Major device is got dynamically:%d\n",scull_major);
	}
	if (result < 0) {
